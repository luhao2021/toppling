...
class ToppleExecutor:
    ...
    @staticmethod
    def open_gripper():
        rospy.loginfo("[GRIPPER] open: rosrun xarm_gripper gripper_client 850 1500")
        os.system("rosrun xarm_gripper gripper_client 850 1500")
        time.sleep(0.5)

    @staticmethod
    def close_gripper():
        rospy.loginfo("[GRIPPER] close: rosrun xarm_gripper gripper_client 440 1500")
        os.system("rosrun xarm_gripper gripper_client 440 1500")
        time.sleep(2.0)
    ...
    def _set_pose_target_and_exec(self, pose: Pose, stage_name: str = "") -> bool:
        if stage_name:
            rospy.loginfo(f"[STAGE] {stage_name}: _set_pose_target_and_exec -> {self._pose_str(pose)}")
        else:
            rospy.loginfo(f"[STAGE] _set_pose_target_and_exec -> {self._pose_str(pose)}")
        self.group.set_pose_target(pose)
        ok = self.group.go(wait=True)
        self.group.stop()
        self.group.clear_pose_targets()
        rospy.loginfo(f"[STAGE] {stage_name or '_set_pose_target_and_exec'} result={bool(ok)}")
        return bool(ok)
    ...
    def _cartesian_push(self, dx: float, dy: float, dz: float = 0.0, stage_name: str = "") -> bool:
        rospy.loginfo(
            f"[STAGE] {stage_name or '_cartesian_push'}: delta=({dx:.3f}, {dy:.3f}, {dz:.3f})"
        )
        waypoints = []
        ...
        tgt.position.y += dy
        tgt.position.z += dz
        rospy.loginfo(f"[STAGE] {stage_name or '_cartesian_push'} target -> {self._pose_str(tgt)}")
        waypoints.append(tgt)
        plan, fraction = self.group.compute_cartesian_path(waypoints, 0.005, 0.0)
        rospy.loginfo(f"[STAGE] {stage_name or '_cartesian_push'} cartesian fraction={fraction:.3f}")
        if fraction < 0.95:
            rospy.logwarn(f"[STAGE] {stage_name or '_cartesian_push'} failed: fraction < 0.95")
            return False
        ok = bool(self.group.execute(plan, wait=True))
        rospy.loginfo(f"[STAGE] {stage_name or '_cartesian_push'} result={ok}")
        return ok

    def execute_move(self, block: str, src_pose: Pose, dst_pose: Pose, safe_lift: float = 0.10) -> bool:
        rospy.loginfo(f"[ACTION][Move] block={block}")
        rospy.loginfo(f"[ACTION][Move] src(center) -> {self._pose_str(src_pose)}")
        rospy.loginfo(f"[ACTION][Move] dst(target) -> {self._pose_str(dst_pose)}")
        ...
        # Required gripper sequence: open before pick -> close at grasp -> open at place
        self.open_gripper()

        if not self._set_pose_target_and_exec(pre_grasp, stage_name="move/pre_grasp"):
            return False
        if not self._set_pose_target_and_exec(grasp, stage_name="move/grasp"):
            return False

        self.close_gripper()

        if not self._set_pose_target_and_exec(pre_grasp, stage_name="move/lift"):
            return False
        if not self._set_pose_target_and_exec(pre_place, stage_name="move/pre_place"):
            return False
        if not self._set_pose_target_and_exec(place, stage_name="move/place"):
            return False

        self.open_gripper()

        if not self._set_pose_target_and_exec(pre_place, stage_name="move/retreat"):
            return False
        return True
    ...
    def execute_smash(
        ...
        rospy.loginfo(f"[ACTION][Smash] direction={direction}, base_pose={self._pose_str(block_pose)}")
        direction_map = {
            "+x": (1.0, 0.0, 0.0),
            "-x": (-1.0, 0.0, math.pi),
        ...
        if not self._set_pose_target_and_exec(pre, stage_name="smash/pre_approach"):
            return False
        if not self._set_pose_target_and_exec(touch, stage_name="smash/touch"):
            return False
        if not self._cartesian_push(ux * push_dist, uy * push_dist, 0.0, stage_name="smash/push"):
            return False
        return self._cartesian_push(-ux * retreat, -uy * retreat, 0.02, stage_name="smash/retreat")
...
def run_problem_once(yaml_path: str, executor: ToppleExecutor, provider: AprilTagProvider):
    ...
    for i, action in enumerate(actions):
        ...
        if typ == "Move":
            ...
            rospy.loginfo(
                f"[ACTION][Move] block={name}, from={action.get('from')}, to={action.get('to')}, "
                f"detected_center={executor._pose_str(src)}"
            )
            ...
            rospy.loginfo(
                f"[ACTION][Smash] blocks={blocks}, dir={dirn}, from={action.get('from')}, "
                f"detected_base_center={executor._pose_str(base_pose)}"
            )
